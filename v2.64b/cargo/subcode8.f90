...
!cargo flux file for cm12 and 18f
! group  mesh#   flux
subroutine WriteCargo
use paraset1
use paraset2
use paraset3
use paraset4
...
integer g,i, n_mesh(3)
real mflux

cur_filename='cargo.cm12'
open (unit=43,file=cur_filename)
write(43, "('group    mesh     flux')")
cmk=1
cmi=2
cmj=3
n_mesh=zlevel(cmk)%cm_zlev(cmi,cmj)%fine
...
do fmk=1,n_mesh(3)
do fmj=1, n_mesh(2)
mflux=mflux+zlevel(cmk)%cm_zlev(cmi,cmj)%fluxset(1)%locg_set(g)%locg_flux(fmi,fmj,fmk)

enddo  !fmj
enddo  !fmk

write(43,"(I4,4x,I4,4x,ES12.5)") g,fmi,mflux/n_mesh(2)/n_mesh(3)
...
write(43,"(I4,4x,I4,4x,ES12.5)") g,fmj,mflux/n_mesh(3)/n_mesh(1)
