...
! output adjoint flux for array project fov ajoint model
Subroutine WriteFov
use files
use funs
use ErrControl
...
set_number=1
pnt=0
allocate (tempbuf(num_group))

open(unit=34, file='fov.dat')
write(34,"(' Pnt   CMesh   FMesh   finesize   position    importance' )")
...
do g=1, num_group
grp=num_group-g+1
tempbuf(g)=tempbuf(g) + zlevel(k)%cm_zlev(i,j)%fluxset(set_number)%locg_set(grp)%locg_flux(ini,inj,ink)

enddo !grp
enddo !ink
enddo !ini

write(34,"(3(I4,4x), f6.2,2x, f10.3, 3x, 300(ES12.5,2x)  )") pnt, j, inj, & 
   zlevel(k)%cm_zlev(i,j)%finsize(2), zlevel(k)%cm_zlev(i,j)%cm_y(inj), tempbuf 
