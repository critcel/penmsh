...
real :: pnt_pix
integer :: curg_grp=1, grp_digit=0
...
if(num_group .lt. 10) then
grp_digit=1
else if( num_group .lt. 100) then
grp_digit=2
else if( num_group .lt. 1000) then
grp_digit=3
else if( num_group .lt. 10000) then
grp_digit=4
else
grp_digit=0
endif
...
integer :: set_number=1
real :: fm_xyz(3), lim_val(2)=0, x_d(2), y_d(2)
real ::  viewpnt(3), view_ang_offset, view_dist

character(LEN=20) :: what2plot(3)
logical ex
real :: fseg

!plot mat, src, or flx
what2plot(1)='Mat Num'
what2plot(2)='Fixed Src'
what2plot(3)='Flux' 
...
write(form,"('(A, A1, I', I0,'.', I0,  ')' )") grp_digit, grp_digit 
select case( ipx)
case (1)
...
case (3)
  write(curfile%name, form) trim(cstr),'f', curg_grp   
end select
...
!offset view angle by 45 degree
view_ang_offset=view_ang+45
view_ang_offset=view_ang_offset/180*pi

view_dist=0
do i=1,2
 view_dist=view_dist+view_pos(i)**2
enddo
view_dist=view_dist**0.5

viewpnt(1)=view_dist*cos(view_ang_offset)
viewpnt(2)=view_dist*sin(view_ang_offset)
if(view_ang .ge. 0 ) then
viewpnt(3)=view_pos(3)
else
viewpnt(3)=-view_pos(3)
endif
